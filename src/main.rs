...
pub fn detect_movement_and_heading(sim: &mujoco_rust::Simulation, last_qpos: Vec<f64>) -> (bool, bool) {
    let qpos = sim.qpos();
...
fn main() {
    ...
    let model = mujoco_rust::Model::from_xml("/home/m3/project/myRustPilot/src/x2/scene.xml".to_string()).unwrap();
    ...
    // test
    // for i in 0..nsensor {
    //     let name_idx = unsafe { *mj_model.name_sensoradr.add(i.try_into().unwrap()) as usize };
    //     let name = unsafe {
    //         std::ffi::CStr::from_ptr(mj_model.names.add(name_idx))
    //             .to_str()
    //             .unwrap()
    //             .to_owned()
    //     };
    //     // sensor_names.push((i, name));
    //     println!("sensor {}: {}", i, name);
    //     let dim = unsafe { *mj_model.sensor_dim.add(i as usize) } as usize;
    //     let adr = unsafe { *mj_model.sensor_adr.add(i as usize) } as usize;
    //     let data = &simulation.sensordata()[adr..adr+dim];
    //     println!("{}:{} {:?}", name, adr, data);
    // }
    ...
    for i in 0..500 { // 模拟运行 500 步，即 5 秒 (500 * 0.01s)
        ...
        //     println!("geom {} (name={}): position = {:?}", i, geom.name, xpos[i]);
        // }
        let gyro_data = &simulation.sensordata()[gyro_start..gyro_start + gyro_dim];
        let acc_data = &simulation.sensordata()[acc_start..acc_start + acc_dim];
        let att_data = &simulation.sensordata()[att_start..att_start + att_dim];
        ...
        if i % 100 == 0 {
            let last_qpos = simulation.qpos();
            // camera::get_camera_jpg(&simulation, 1080, 1920);
            println!("Position: ?{:?}", last_qpos);   
